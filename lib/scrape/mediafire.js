...
function mediafire2(url) {
	return new Promise((res, rej) => {
		...
		axios(url).then(async(c) => {
			...
			let fileName = $('div.dl-btn-label').attr('title')
			let fileSize = $('a#downloadButton').text().split('\n')[1].replace(/ /g, '').replace(/\(|\)/g, '').replace('Download', '')
			let downloadUrl = $('a#downloadButton').attr('href')
			let mimetype = await Mime.lookup(downloadUrl)
			res({ fileName, fileSize, mimetype, downloadUrl })
		}).catch(rej)
	})
}
