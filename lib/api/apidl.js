					...
					axios.get(source[Math.floor(Math.random() * source.length)])
						.then(({
							...
							result = {
								...
								url: source[Math.floor(Math.random() * source.length)],
								sticker_url: link
							}
							resolve(result)
						})
...
async function stickersearch2(querry) {
...
async function emoji(emoticon) {
	...
	axios.get(`https://emojipedia.org/${emojii}/`)
            .then(({data}) => {
	const $ = cheerio.load(data)
...
module.exports.stickersearch2  = stickersearch2
