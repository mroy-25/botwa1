...
const { serialize, smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetchJson, await, sleep } = require('./lib/myfunc')
...
async function startbot() {
    ...
    let blockxeon = await conn.sendContact(callerId, global.owner)
    conn.sendMessage(callerId, { text: `*Automatic blocking system!*\n*Don't call bot*!\n*Please contact the owner to open block !*`}, { quoted : blockxeon })
    await sleep(8000)
    await conn.updateBlockStatus(callerId, "block")
    }
    })
    conn.mode = 'public'
    conn.multi = global.multiprefix
    conn.nopref = global.noprefix
    conn.prefa = global.prefix
    ...
    conn.ev.on('messages.upsert', async chatUpdate => {
        ...
        msg = serialize(conn, mek)
        m = smsg(conn, mek, store)
        require("./message")(conn, m, msg, chatUpdate, store)
        } catch (e) {
            console.log(e)
        }
    })
...
conn.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
    ...
    // Add Other
     ...
     return conn.sendMessage(jid, { audio: await getBuffer(url), caption: caption, mimetype: 'audio/mpeg', ...options}, { quoted: quoted, ...options })
      ...
      if(mime.split("/")[0] === "sticker"){
     return conn.sendMessage(jid, { sticker: await getBuffer(url), caption: caption, ...options}, { quoted: quoted, ...options })
      }
      }
